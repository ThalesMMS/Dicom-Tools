...
// Dicom-Tools-rs
//
// Tests for navigation in nested DICOM structures (SR Content Sequences).
//
...
fn test_value_at_path_sequence_navigation() {
    ...
    // Navigate to ContentSequence -> first item -> ConceptNameCodeSequence -> first item -> CodeMeaning
    let path = vec![
...
fn test_value_at_path_nested_content_sequence() {
    ...
    // Navigate through three CONTENT_SEQUENCE hops before reaching TEXT_VALUE.
    let path = vec![
        PathSegment::element(tags::CONTENT_SEQUENCE),
        PathSegment::element(tags::CONTENT_SEQUENCE),
        PathSegment::element(tags::CONTENT_SEQUENCE),
...
fn test_value_at_path_invalid_path() {
    ...
    // Try to navigate to an element that doesn't exist
